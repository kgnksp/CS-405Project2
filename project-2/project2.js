...
class MeshDrawer {
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			// These are the parameters for textures with power of 2 dimensions.
			...
			// These are the parameters for non-power of 2 textures.
    ...
    gl.useProgram(this.prog);
    // Set the uniform to reflect the checkbox state
	...
	setAmbientLight(intensity) {
		// Always update the uniform, but its effect will be conditional on enableLighting in the shader.
		this.ambientIntensity = intensity;
		gl.useProgram(this.prog);
		gl.uniform1f(this.ambientLoc, intensity);
	}
...
void main() {
    ...
    // Initialize final color with ambient color regardless of lighting being enabled.
    

    // Only apply lighting if it's enabled.
    if (enableLighting) {
		vec3 norm = normalize(v_normal);
		vec3 finalColor = baseColor.rgb * ambient;
        vec3 lightDir = normalize(lightPos);
        float diff = max(dot(norm, -lightDir), 0.0);
        finalColor += (diff * baseColor.rgb);
